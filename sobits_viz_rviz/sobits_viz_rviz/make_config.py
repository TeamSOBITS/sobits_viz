...
def _panels() -> list:
    # Help Height 0 drops the description box under the display tree.
    return [
        {'Class': 'rviz_common/Displays', 'Name': 'Displays', 'Help Height': 0,
...
def _dock_state(left: list, right: list, bottom: list) -> str:
    ...
    strip = docked(bottom, Qt.BottomDockWidgetArea)
    if panels:
    ...
    if strip:
        window.resizeDocks(strip, [BOTTOM_DOCK_PX] * len(strip), Qt.Vertical)
...
def _window(displays: list) -> dict:
    ...
    names = [panel['Name'] for panel in _panels()]
    panels = [name for name in names if name not in BOTTOM_PANELS]
    strip = [name for name in names if name in BOTTOM_PANELS]
    ...
    state = _dock_state(panels, images, strip)
    if state:
